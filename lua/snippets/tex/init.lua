...
local utils = require("snippets.tex.util.utils")
...
local _autosnippets = function(is_math, not_math)
  ...
  for _, snip in ipairs(require("snippets.tex.math_wRA_no_backslash")) do
    snip.trig_matcher = match_pattern
    snip.condition = pipe({ is_math, no_backslash })
    table.insert(autosnippets, snip)
  end

  for _, snip in ipairs(require("snippets.tex.math_rA_no_backslash")) do
    snip.wordTrig = false
    snip.trig_matcher = match_pattern
    snip.condition = pipe({ is_math, no_backslash })
    table.insert(autosnippets, snip)
  ...
  for _, snip in ipairs(require("snippets.tex.normal_wA")) do
    snip.condition = pipe({ not_math })
    table.insert(autosnippets, snip)
  end

  for _, snip in ipairs(require("snippets.tex.math_wrA")) do
    snip.trig_matcher = match_pattern
    snip.condition = pipe({ is_math })
    table.insert(autosnippets, snip)
  end

  for _, snip in ipairs(require("snippets.tex.math_wA_no_backslash")) do
    snip.condition = pipe({ is_math, no_backslash })
    table.insert(autosnippets, snip)
  end

  for _, snip in ipairs(require("snippets.tex.math_iA")) do
    snip.wordTrig = false
    snip.condition = pipe({ is_math, no_backslash })
    table.insert(autosnippets, snip)
  end

  for _, snip in ipairs(require("snippets.tex.math_iA_no_backslash")) do
    snip.wordTrig = false
    snip.condition = pipe({ is_math, no_backslash })
    table.insert(autosnippets, snip)
  end

  for _, snip in ipairs(require("snippets.tex.math_bwA")) do
    snip.condition = pipe({ conds.line_begin, is_math })
    table.insert(autosnippets, snip)
  end

  for _, snip in ipairs(require("snippets.tex.bwA")) do
...
M.setup_tex = function(is_math, not_math)
  ...
  local math_i = require("snippets.tex.math_i")
...
M.setup_markdown = function()
  ...
  local normal_wA = vim.tbl_map(trigger_of_snip, require("snippets.tex.normal_wA"))
  local bwa = vim.tbl_map(trigger_of_snip, require("snippets.tex.bwA"))
